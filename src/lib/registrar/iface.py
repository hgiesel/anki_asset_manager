...
def _make_loose_interface(tag: str) -> Interface:
    return make_interface(
        ...
        getter=lambda id, storage: make_script_v2(
            ...
            if storage.enabled is not None
            else loose_script.enabled,
            ...
            if storage.version is not None
            else loose_script.version,
            description=storage.description
            if storage.description is not None
            else loose_script.description,
            position=storage.position
            if storage.position is not None
            else loose_script.position,
            conditions=storage.conditions
            if storage.conditions is not None
            else loose_script.conditions,
